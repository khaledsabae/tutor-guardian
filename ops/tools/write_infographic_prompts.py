...
# 🚨 فئة لم تكن في الحسبان: **التصوير لا الاقتباس**.
#
# أنتج النموذج قسمًا نصّه «رسم يد طفل ممسكة بيد كبيرة **(رمز يد الله)**» —
# تمثيلٌ لذات الله يُرسَم في صورة تصل الأهل والأطفال. وكل بواباتي كانت تسأل
# «هل يقتبس نصًّا؟»، ولا واحدة تسأل «هل يصوّر ما لا يُصوَّر؟». وأنتج قسمًا آخر
# فيه «آية **غير مقتبسة** مكتوبة بخط جميل» — تعليمة متناقضة تفتح لمولّد الصور
# باب اختراع آية.
#
# هذه لا تُعرَض للمراجعة بل **تُرفض**: خطؤها ليس في درجة النسبة بل في أصلها،
# ولا يحتاج تمييزُه معرفةً بالمتون.
DEPICTION = re.compile(
    r"(رمز|رسم|صورة|تمثيل|شكل)[^.\n]{0,30}(يد|وجه|ذات|هيئة)\s*(الله|الرب)|"
    r"يد الله|وجه الله|تجسيد\s*(الله|الذات)|"
    r"آية\s*(غير\s*مقتبسة|مؤلفة|من عندك|مخترعة)|حديث\s*(غير\s*مقتبس|مؤلف)")
...
def draft(lesson_id: str) -> dict:
    ...
    dep = DEPICTION.search(blob)
    if dep:
        return {"id": lesson_id, "status": "rejected",
                "reason": f"تصويرٌ ممنوع أو نصّ مخترَع: «{dep.group(0)[:44]}»"}
