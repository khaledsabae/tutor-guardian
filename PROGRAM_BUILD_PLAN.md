...
| 2026-06-10 | **P0.4 — توسيع keyword fast-path في domain_classifier** | ✅ | Commit `245ffaa`. **التأثير**: Baseline 61/100 (61%) → Final 100/100 (100%) — تجاوزنا الهدف 80% بكثير. **التغييرات**: Fiqh (كذب/يسرق/يدخن/يحب الموسيقى/يسب الدين/يحبب/يحرم/...)، Cyber (إنستغرام/فايبر/فكونتاكت/تليجرام/محتوى عنيف/سيء/تواصل مع غريب)، Medical (ADHD/سمنة/يأكل التراب/يشد شعرها/كثير البكاء/يخاف من الناس/يرفض الأكل/...)، Development (لا يبتسم/لا يركض/لا يستعمل الحمام/لا يكلم/...). **اختبارات جديدة**: 5 tests (4 tests لكل domain + 1 contract test على 100 سؤال، ≥80% min, ≥90% reality). **False positive تجنبت**: 'غريب' كان يطابق cyber → rephrase 'تواصل مع غريب'/'غريب على النت'، 'كلمات من النت' انتقل من development لـ cyber. **الـambiguous test** (بنتي عمرها 5 سنوات تصرفاتها غريبة) لا يزال يذهب لـ LLM (صحيح). **البوابات**: pytest 111/111 ✓ (108 سابقة + 3 جديدة)، flutter analyze 0 errors، flutter test 84/84 ✓، KB integrity 292 ✓. |
| 2026-06-10 | **P0.1 — مشغّل البودكاست الحقيقي (ثالث placeholder يتشال — P0.1 كتمل)** | ✅ | Commit `757e6e5`. أضفت `just_audio` dependency. شاشة PodcastPlayerScreen كاملة (350 سطر): URL مباشر أو relative path مع AppConfig.apiBaseUrl، null/empty URL → رسالة 'سيتاح قريباً'، play/pause (72×72 زر دائري)، seek bar + position/duration، rewind 10s + forward 30s، Speed cycling: 1× / 1.25× / 1.5× / 1.75× / 2×، loading + error states. lesson_screen.dart: podcast placeholder → PodcastPlayerScreen. **3 widget tests** (null/empty URL + speed constants). 3 اختبارات موجودة تعديلت (placeholder text استبدل بـtitle). البوابات: `flutter analyze` 0 errors ✅، `flutter test` **84/84** ✅ (81 سابقة + 3 جديدة)، KB integrity 292 ✓، pytest لا-smoke أخضر ✓. **P0.1 كتمل 100% — كل الـ3 placeholders (video/podcast/quiz) استُبدلت**. |
| 2026-06-10 | **P0.1 — مشغّل الكويز الحقيقي (ثاني placeholder يتشال)** | ✅ | Commit `79a41ba`. QuizDeck model + Repository.getQuizDeck + quizDecksProvider + شاشة QuizScreen كاملة (519 سطر) + 7 widget tests جديدة. **الـflow**: سؤال مع شريط تقدم → 4 خيارات مع حروف أبجدية → tap يقفل الإجابة + feedback فوري (أخضر/أحمر + check/cancel icons) + rationale card + hint card → next/Summary → شاشة ملخص بنتيجة + حكم + retry. lesson_screen.dart: كويز placeholder → QuizScreen. **الـbackend لم يتغير** (نفس `getAssetContent` endpoint، JSON يحدد kind=quizzes). البوابات: `flutter analyze` 0 errors ✅، `flutter test` 81/81 (74 سابقة + 7 جديدة) ✅، KB integrity 292 ✓، pytest لا-smoke أخضر ✓. **الثالث والأخير placeholder المتبقي**: البودكاست. |
| 2026-06-10 | **P0.1/P2.2 — حزمة مصادر NotebookLM مكتملة لـ 14 درس 0-3** | ✅ | Commit `4ac9bf6`. 12 ملف `.md` + README في `notebooklm_sources/0-3/` — كل درس من دروس الـ0-3 الـ14 يحتوي ملف مصدر جاهز للرفع على Google NotebookLM وتوليد **Audio Overview بالعربية** (بودكاست MP3 لكل درس). البوابات: CJK + Cyrillic gate نظيف (0 violations)، KB integrity 292 ✓، pytest (لا-smoke) أخضر. **المحتوى**: 4 مسارات (medical 4 + islamic 4 + cyber 3 + development 3). كل ملف: lesson summary + try_this + reflection prompts + مراجع حقيقية (CDC, WHO, AAP, Bowlby, Vygotsky, علوان، ابن القيم، الألباني). **التالي:** خالد يرفعها على NotebookLM (ليس API، عملية يدوية بـ14 نوتبوك) → Audio Overview AR → MP3 → رفع على Cloudflare R2 → ربط `lesson-assets` endpoint. **(نقلتها للحالة 4 من الخطة الجديدة: كويز/بودكاست/فيديو/اداء = الـP0.1+P0.4 + المحتوى).** |
| 2026-06-10 | **P0 تابع: KB كلها معنونة + أول مشغّل ميديا حقيقي (فلاش كاردز) + AAB v2** | ✅ | (1) **دفعات 3+4 (38c493d):** كل الـ116 وحدة المتبقية بلا عنوان حصلت عناوين عربية — **0 وحدة بلا عنوان في كل الـKB** (الصبح كانوا 147). (2) **مشغّل الفلاش كاردز (f26426d):** endpoint جديد additive `GET /api/program/asset-content/{id}` (مع حماية path-traversal + كاش) + شاشة Flutter بقلب 3D وشريط تقدم وpager — أول placeholder يتشال. اختبارات: backend 4 جديدة، Flutter 74/74، analyze نظيف. (3) **AAB أعيد بناؤه** (58.4MB) شامل الفلاش كاردز — جاهز للرفع على Internal testing. (4) **keystore متأمّن** في `~/SECURE_BACKUPS/` خارج الريبو. (5) Deploy السابق أخضر ومتحقق على الإنتاج. ملاحظة: SSH للـVPS (الباك أب الليلي) محجوب يحتاج إذن صريح من خالد. |
| 2026-06-11 | **P1.2 — بطاقة مشاركة النصيحة (Share Card) — محرك النمو** | ✅ | Commit `c2b4e60`. **الميزة**: الأب يحوّل النصيحة اليومية لصورة جميلة (1080×1080→2160×2160@2x) ويشاركها (WhatsApp/Facebook). **التنفيذ**: Added `screenshot:^3.0.0` + `share_plus:^10.0.0`. `ShareableTipCard` (1080×1080، RTL، AppTheme.primary 0xFF1A5F7A، خط Cairo، خلفية متدرجة + أنماط هندسية، أيقونة شمس + نص النصيحة + شريحة وقت + فوتر «المربي الذكي»). `DailyTipCard`: زر «شارك 📤» جنب القلب — يستخدم `ScreenshotController.captureFromWidget(ShareableTipCard, pixelRatio: 2.0)` → PNG → temp file → `share_plus` (text+subject+file). Loading spinner أثناء التوليد، SnackBar خطأ لطيفة. **البوابات**: `flutter analyze` 0 errors ✅، `flutter test` **95/95** ✅ (89 سابقة + 6 جديدة للـshare card)، KB integrity 292 ✅، pytest 111/111 ✅، CJK gate 0 violations. **النشر**: mobile-only، لا backend changes، لا deploy مطلوب.
| 2026-06-11 | **اكتمال المنهج التفاعلي لكل الأعمار 0-18 (Claude)** | ✅ منشور | أُضيفت دروس 0-3 الـ14 (لم تكن في lesson_index) للنظام: المولّد صار يُنشئ مدخل lesson_index تلقائياً للدروس غير الموجودة. أُلِّفت أصول نظيفة عربية لها (شاشات، تطور مبكر، تعلّق نبوي، مراحل طبية). **الإجمالي 59/59 درس** (0-3 → 16-18) بفلاش كاردز + كويز. مُتحقَّق حيّاً على الإنتاج (healthy، resolve عبر loader). صفر مشاكل جودة. المنهج التفاعلي مكتمل لكل الفئات العمرية. متبقٍ: self-hosted runner، تنظيف اليتامى. |
| 2026-06-11 | **كل الـ45 درس بأصول نظيفة حيّة على الإنتاج ✅ (Claude)** | ✅ منشور | اكتمل استبدال أصول NotebookLM المعطوبة بالكامل: **45/45 درس** (كل الأعمار 4-6 → 16-18) لها فلاش كاردز + كويز عربي دقيق مؤلَّف من محتوى كل درس (5 بطاقات + 3-4 أسئلة، إجابة صحيحة واحدة + تعليل + تلميح). صفر مشاكل جودة عبر كل الدفعات. مولّد قابل لإعادة التشغيل `scripts/generate_clean_assets.py`. نُشر على الإنتاج (docker build+restart، healthy) ومُتحقَّق حيّاً عبر `/api/program/lesson-assets` و`/asset-content`. **المحتوى المعطوب اختفى تماماً.** متبقٍ: دروس 0-3 (مش في lesson_index)، self-hosted runner، تنظيف 251 ملف يتيم. |
| 2026-06-10 | **نشر فعلي + توسّع الأصول النظيفة (Claude، SSH بإذن خالد)** | ✅ منشور | SSH للـVPS ونشر يدوي مرتين: (1) شيل المحتوى المعطوب — **اختفى من الإنتاج**. (2) **21 درس بأصول نظيفة حيّة** على tg-api.alsaba.cloud (كل 10-12 + كل 13-15). محتوى حسّاس (بطاقة سلامة + طوارئ 937/112 + DMDD + شبهات دينية) مؤلَّف بعناية ومُتحقَّق حيّاً. الباك إند healthy. **قرارات خالد:** التوكن يتأجّل (تجديده ويب-فقط، مش ممكن برمجياً)؛ الفوترة → self-hosted runner (لسه)؛ القوالب الفاضية → تُملأ بالتوليد النظيف (جارٍ). متبقٍ 24 درس + runner. Gates خضراء كل دفعة. |
| 2026-06-10 | **مراجعة شغل Hermes + اكتشاف أصول معطوبة + إصلاحها (Claude)** | ✅ كود / ⏳ نشر | راجعت دفعات Hermes (كويز/بودكاست/أداء): سليمة ومتحقَّقة. **اكتشاف حرج:** أصول NotebookLM التفاعلية معطوبة — 35/35 كويز نفس السؤال الإنجليزي العام، 292 فلاش بـ39 محتوى مميز فقط (116 قوالب فاضية)، و38/39 deck على الـ6 دروس الحية كانت تعرض إنجليزي عام على دروس عربية. **الإصلاح:** (1) إخفاء كل fc/qz المعطوب من lesson_index (commit ecc4600). (2) مولّد أصول نظيفة `scripts/generate_clean_assets.py` + تأليف يدوي عربي دقيق من محتوى كل درس — **8 دروس** (pilot 3 + كل الـ5 cyber اللي كانت معطوبة-حية) بـ5 بطاقات + 3-4 أسئلة لكل درس، كل سؤال إجابة صحيحة واحدة + rationale + hint (b3e6345, 3915691). بوابة النقاء (CJK/كيريلي) في المولّد. Gates خضراء (KB 292، pytest، جودة 0 مشاكل). **بلاغان لخالد:** توكن GitHub مسرّب (تجديد فوري) + فوترة Actions موقوفة (→ self-hosted runner). **⏳ كل ده على main لكن غير منشور — الـCI ميت، النشر يحتاج SSH للـVPS.** |
| 2026-06-10 | **P0 — يوم التنظيف الكبير (Claude منفّذاً منفرداً):** KB نظيفة 100% من CJK/الكيريلي + بوابة مشددة + إصلاح RAG + أصول الإنتاج | ✅ | (1) **33 وحدة ملوثة بالـCJK أُصلحت كلها** (18 جراحية + 15 إعادة كتابة عميقة): اكتُشف أن وحدات isl-* لها نصوص أصلية عربية معكوسة (OCR) وملخصات صينية **مفبركة** لا علاقة لها بالمصدر (تدّعي محتوى فرط حركة على نصوص تربية إسلامية!) — أعيدت كتابتها بأمانة. tip_7-9_022 كانت مبنية على محتوى مفبرك وصُححت. (2) **بوابة النقاء**: الوحدات صارت blocking (كانت تحذيراً) + أضيف الكيريلي للنطاق بعد اكتشاف 'поиск' روسية في درس إنتاج. (3) **إصلاح RAG**: وحدات unspecified (65) كانت غير مرئية في الاستعلام الأساسي — الفلتر صار $in [age, unspecified]. (4) **دفعة جودة 2**: كل وحدات medical الـ21 الباقية بلا عنوان حصلت عناوين عربية + إصلاح ترجمات خطيرة (نص يقول 'إطلاق النار على الآخرين' كترجمة لأعراض DMDD!) + سن DMDD مصحح (لا تشخيص قبل 6 سنوات). (5) **أصول المنتج**: 292 فلاش كارد + 90 سلايد دخلت git (كانت ناقصة من صورة الإنتاج رغم أن lesson_index يشير لها) + تجاهل 9.4GB مخرجات Kaggle. (6) **حزمة برومبتات الفيديوهات** `docs/arabic_video_prompts.md`: 9 برومبتات عربية جاهزة للصق في NotebookLM. **الكوميتات:** 8bad13f→102bd80 (8 دفعات). Gates خضراء في كل دفعة. |
| 2026-06-10 | **الخطة العالمية P0 بدأت (Claude منفّذاً):** Package 2 مكتمل 100% على الإنتاج + AAB جديد + KB pilot batch 1 | ✅ | (1) **Package 2 closed:** المسار الرابع `path_0-3_development_early_moments` مدفوع (4fbfd16) ومتحقَّق على الإنتاج — 4 مسارات 0-3 live. (2) **AAB للاختبار الداخلي:** بناء جديد 58.3MB بآخر إصلاحات mobile (analyze نظيف + 69/69 tests) — `mobile/build/app/outputs/bundle/release/app-release.aab` جاهز لرفع خالد على Play Console (الاختبار المغلق يتطلب 12 مختبراً × 14 يوماً). (3) **KB quality pilot batch 1 (8bad13f + c0af001):** مسح شامل كشف 169 وحدة بمشاكل (147 بلا عنوان، 65 بلا عمر، تلوث CJK في نصوص مُقدَّمة للـRAG). أُصلحت 10 وحدات medical: عناوين عربية + إعادة كتابة 4 نصوص ملوثة/مشوهة بأمانة من text_original + إصلاح keywords صينية. Gates: integrity 292 ✓ + pytest 102/102 ✓. **متبقٍ: 159 وحدة على دفعات.** |
...
| 2026-06-10 | Package 2 — محتوى 0-3: المسار الطبي ✅ + المسار الإسلامي ✅ | ✅ | **مساران من 3-4 مكتملين للـ0-3 على الإنتاج (tg-api.alsaba.cloud):** (1) **medical** — path_0-3_medical_early_milestones (4 دروس + 5 نصائح، المراحل النمائية + علامات التنبيه المبكر + التدخل المبكر، piloto). (2) **islamic_parenting** — path_0-3_islamic_parenting_attachment (4 دروس + 5 نصائح، سنن الاستقبال/التسمية/العقيقة، الرفق والرحمة، التواصل والتعلق الآمن، بذر الإيمان بالبيئة والتكرار، الإطار prophetic_7_7_7). كلاهما يستخدم unit_ids حقيقية من الـ27 وحدة KB للـ0-3. بوابات: CJK=0 ✅، Schema=0 ✅، Cross-ref=0/0 ✅، pytest=22/22 ✅، Production 200 مع 2 paths. **متبقٍ لـ0-3:** cyber (الشاشات/الأجهزة للرضّع، 5 وحدات) + development اختياري (3 وحدات). |
| 2026-06-10 | Package 1 — نشر نموذج tg-tutor:v2 (Phase 1 فقط) | ✅ | **تم بنجاح على الإنتاج (tg-api.alsaba.cloud):** (1) تنزيل نموذج التطوير من Kaggle (tg-tutor-v1-Q4_K_M.gguf ~1.8GB + Modelfile.tg-tutor). (2) رفع للـhome server عبر VPS. (3) إنشاء نموذج Ollama `tg-tutor:v2` على الـhome server (100.109.163.64:11434). (4) Smoke test: 3 أسئلة تربية عربية — الإجابات عربية ومعقولة وآمنة. (5) تعديل `OLLAMA_PRIMARY_MODEL=tg-tutor:v2` في `.env` على الـVPS + `docker compose up -d backend`. (6) تحقق: `tg_backend == healthy` ✅، `curl /health` → 200 ✅، endpoint `/api/assistant/draft` (يستخدم `generate()`) يرجع `mode: llm_generated` ✅، التليمتري يثبت استدعاء `tg-tutor:v2` (latency 26.5s، prompt 737، completion 191، ok=1) ✅. **الـstreaming (SSE) لا يزال على `qwen2.5:3b` السريع كما هو مخطّط — المرحلة 2 متروكة لقرار لاحق.** KB integrity: 292 units in sync. Backend: 110/110 tests pass. |
