...
for a in audios:
    ...
    # CORRECT syntax: -a <artifact_id> then output path
    cmd = [CLI, "download", "audio", "-a", aid, "-n", NOTEBOOK, fpath, "--force"]
    try:
        ...
        if res.returncode == 0 and os.path.exists(fpath) and os.path.getsize(fpath) > 100000:
            ...
            print(f"  [FAIL] {title[:50]} | rc={res.returncode} | {res.stderr.strip()[:100]}")
            failed += 1
