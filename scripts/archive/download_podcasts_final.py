import json, os, subprocess, re, glob, sys
...
print(f"Total completed audio artifacts: {len(audios)}", flush=True)
...
for p in glob.glob(f"{DOCS}/lesson_*_podcast.mp3") + glob.glob(f"{DOCS}/lesson_*_podcast*.mp3") + glob.glob(f"{DOCS}/podcast_*.mp3"):
    ...
    if os.path.getsize(p) < 100000:
        ...
        continue
    existing.add(bn)
print(f"Existing valid podcast files: {len(existing)}", flush=True)
...
for a in audios:
    ...
    try:
        ...
        if res.returncode == 0 and os.path.exists(fpath) and os.path.getsize(fpath) > 100000:
            print(f"  [OK] {title[:50]} ({os.path.getsize(fpath)//1024//1024} MB)", flush=True)
            downloaded += 1
        else:
            err = res.stderr.strip()[:120]
            print(f"  [FAIL] {title[:50]} | rc={res.returncode} | {err}", flush=True)
            failed += 1
    except Exception as e:
        print(f"  [ERR] {title[:50]} | {str(e)[:100]}", flush=True)
        failed += 1
...
final_count = len([p for p in glob.glob(f'{DOCS}/lesson_*_podcast*') + glob.glob(f'{DOCS}/podcast_*') if os.path.getsize(p) > 100000])
print(f"Total valid files now: {final_count}", flush=True)
