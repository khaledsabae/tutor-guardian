...
| 2026-06-08 | Phase 5 | ✅ مكتمل + محقق | **Native identity**: `flutter_launcher_icons` (reusing `frontend/icons/icon-512.png` — متّسق بصرياً مع PWA) + `flutter_native_splash` (brand teal #01696F، splash icon في كل density + Android 12 splash API). Adaptive icon (background=#01696F + foreground=shield) في `mipmap-anydpi-v26/`. iOS app icons regenerated. **Offline handling**: `lib/state/connectivity_provider.dart` (Riverpod StreamProvider من `connectivity_plus`، يصدّر true/false) + `_OfflineBanner` widget (أيقونة wifi_off + رسالة «غير متصل بالإنترنت» بـ warning color) + `ChatNotifier.sendMessage` يقصر المسار برسالة عربية ودودة إذا `!isOnline()`. **Network security**: `network_security_config.xml` يحظر cleartext افتراضياً ويسمح به فقط لـ localhost/10.0.2.2/127.0.0.1 (dev). **Loading state**: `_BootSplash` («جاري تهيئة الجلسة…» + spinner) يظهر عند `state.sessionId == null` (أي أثناء bootstrap). **Signing prep (Phase 6)**: keystore مولّد (`mobile/android/app/almorabbi-upload.jks`، 10000 يوم، JKS) + `key.properties` (مستبعد من git) + `build.gradle.kts` يقرأها ويضبط `signingConfigs.release`. **تحقق:** `flutter analyze` نظيف، `flutter test` 11/11 نجح، `flutter build appbundle --release --dart-define=API_BASE_URL=http://10.0.2.2:8090` → `app-release.aab` 53MB في 110.6s، `META-INF/ALMORABB.RSA` موجود في الـ AAB (موقّع بالـ keystore الفعلي). |
| 2026-06-08 | Phase 0 | ✅ مكتمل (خارج نطاق الوكيل) | خالد كشف الـ backend على HTTPS عام باستخدام **Cloudflare Tunnel** (مش nginx/Let's Encrypt كما اقترحت الخطة الأصلية): container اسمه `tg_cloudflared` على الـ VPS يعمل tunnel مُدار من Cloudflare (tunnel ID `03b1f798-c9cf-4a39-8883-bbd3271a28e8`) → `http://tg_backend:8000`. مفيش nginx، مفيش Let's Encrypt cert، مفيش بورت مفتوح على الـ VPS. الـ TLS عند Cloudflare edge، والـ DNS CNAME (مش A record) → `<tunnel-id>.cfargotunnel.com`. إعداد الـ hostname → service مُدار في Cloudflare Zero Trust dashboard. **تحقق:** `curl https://tg-api.alsaba.cloud/health` → `{"status":"ok"}` (HTTP 200, 370ms). الـ SSE يصل بدون buffering. |
| 2026-06-08 | Phase 6 | ✅ مكتمل + مُسلَّم | **Release AAB النهائي**: `flutter build appbundle --release --dart-define=API_BASE_URL=https://tg-api.alsaba.cloud` → `build/app/outputs/bundle/release/app-release.aab` (53MB) في 26.5s (incremental). **التحقق من الـ AAB:** (1) موقّع بـ keystore الفعلي: `META-INF/ALMORABB.RSA` موجود، `Built-By: Signflinger`، SHA-256 fingerprint مسجّل. (2) الـ base URL `https://tg-api.alsaba.cloud` مضمن كـ compile-time constant في كل الـ ABIs الثلاثة (arm64-v8a, armeabi-v7a, x86_64) — تأكدت عبر `strings libapp.so`. (3) لا dev URL (`10.0.2.2`) موجود في الـ binary. (4) `flutter test` 11/11 نجح + `flutter analyze` نظيف. **جاهز للـ Google Play Console → Internal testing.** الـ AAB موقّع بالـ `almorabbi-upload.jks` المُولَّد في Phase 5 (RSA 2048، 10000 يوم) — يحفظ خالد keystore + password في password manager. الـ release final مربوط بـ backend production (HTTPS-only، cert صالح 90 يوم). ملاحظة: الردود قد تكون بطيئة مؤقتاً لأن home server مشغول بـ dataset generation؛ ستتحسن لاحقاً. |
