...
| 2026-06-08 | Phase 0 | ✅ مكتمل (خارج نطاق الوكيل) | خالد كشف الـ backend على HTTPS عام: `https://tg-api.alsaba.cloud` (DNS A record → 72.62.44.131، nginx reverse proxy → `tg_backend:8000` بإعدادات SSE الصحيحة: `proxy_buffering off` + `proxy_http_version 1.1` + `X-Accel-Buffering: no` + `proxy_read_timeout 300s`، شهادة TLS عبر Let's Encrypt wildcard `*.alsaba.cloud` صالحة حتى 2026-09-04 باصدار Google Trust Services WE1). **تحقق:** `curl https://tg-api.alsaba.cloud/health` → `{"status":"ok"}` (HTTP 200, 370ms). الـ SSE يصل بدون buffering (تم اختبار streaming حي). |
