...
> **سطر تقرير الفجوات — مُضاف 2026-08-13** (نسخة الجدول قبل الإضافة في
> `/root/crontab-backup-20260813T141013Z.txt`):
>
> ```
> 0 6 * * 6 docker exec -w /app tg_backend python ops/scripts/weekly_kb_gap_report.py >> /var/log/tg-kb-gaps.log 2>&1
> ```
>
> 🚨 **`/app/ops` عبارة عن volume يحجب `ops/tools` بتاع الصورة.** دوكر نسخ
> المجلد داخل الـvolume مرة واحدة يوم إنشائه ولم يحدّثه بعدها: كانت الحاوية تحمل
> ٢٠ أداة والمستودع ٤٥، ومن الغائب **كل حرّاس المحتوى الشرعي**. و`ops/tools/**`
> داخل فلتر النشر، فالتعديل هناك كان يشغّل نشرًا ينجح ظاهريًا ويشحن لا شيء.
> الحل bind صريح في الـcompose — **وأي مسار جديد تحت `/app/ops` يحتاج نفس
> المعاملة.**
>
> ⚠️ **السكربت يرفض العمل (خروج 2) إن لم يجد `mobile/lib/l10n/app_ar.arb`** — وهو
> يصل الحاوية عبر bind كذلك. الرفض متعمَّد: تقرير بلا استبعاد الاقتراحات الجاهزة
> يبدو طبيعيًا تمامًا وهو يرتّب أزرار التطبيق كأنها طلب آباء. للتجاوز عن عمد:
> `--allow-unfiltered`.
>
> ⚠️ **الأسئلة التي يرجع استرجاعها صفر وحدة لا يراها الحكم** (لا أزواج لها) وهي
> أسوأ الأسئلة حالًا — التقرير يقرأ مخرَج الفحص بجوار الأحكام ويفصل الشكلين. في
> أول تشغيل حقيقي كانت ٢٦ من ١٠٠، والرقم الصحيح ٧٧ غير مخدوم لا ٥١.
>
...
## 10.1) قرار مؤجَّل بدليل — حدّ التحديث الإجباري (2026-08-21)

`MINIMUM_BUILD_NUMBER` **باقٍ على ٨١** رغم أن الإنتاج على Play صار ٩٦ (1.0.51).
والقرار مكتوب هنا لأن البند الوحيد المعلَّق عليه — تفعيل `STORY_AUTH_ENFORCE` —
شرطه المكتوب «بعد رفع الحد فوق ٨١»، فيرجع السؤال كل شهر بلا سبب.

**لماذا لا يُرفع:** رفع الحد تحديثٌ إجباري لكل من هو تحته. والغرض منه هنا سدّ
نداء `/api/program/story` بلا توكن — وهو **ضبط تكلفة لا ثغرة أمان**: النقطة خلف
حدّ معدّل لكل هوية، والمجهول يُحسب على الـIP. إجبار قاعدة مستخدمين على التحديث
مقابل ذلك مبادلة خاسرة.

**ولماذا لا يمكن أصلًا أن يُتّخذ القرار بدليل اليوم:** لا نعرف توزيع البناءات.
`chat_sessions.metadata` فارغة في **٣٧١ من آخر ٤٠٠ جلسة** — `ensureSession()`
ينادي `createSession()` بلا metadata، فالنسخة لا تُسجَّل إلا في مسارات نادرة.
والـ٢٩ الباقية مضلِّلة بدورها: الجلسة تُنشأ مرة واحدة عمر التثبيت ولا تتحدّث بعد
تحديث التطبيق، فجهازٌ ثبّت على 1.0.29 وصار على 1.0.51 ما زال يُحسب 1.0.29 — وهذا
بالضبط سبب ظهور البناءين ٧٤ و٧٥ كأنهما الأغلبية.

**المكان الصحيح للقياس** ليس إنشاء الجلسة بل `push_tokens`: صفّه يُحدَّث كل
إقلاع (`updated_at`)، فعمود `app_version` هناك يعطي تعدادًا حيًّا. يحتاج شقًّا في
الباكند وشقًّا في التطبيق — **ولا يُشحن نصفه**: عمود يُضاف ولا يكتب فيه أحد هو
بالضبط النمط الذي كلّف هذا المشروع مرات (انظر «مسجَّل ≠ يمكن الوصول إليه»).

**متى يُعاد فتح الملف:** حين يوجد قياس حقيقي لتوزيع البناءات، أو حين تظهر إساءة
استخدام فعلية على نقطة القصة تبرّر الكلفة.

### تحديث (2026-09-26): القياس موجود، والأداة جاهزة

التعداد الحي موجود الآن (schema v25: `push_tokens.build_number`، يكتبه التطبيق
مع كل إقلاع منذ ~1.0.56). وظهر سبب جديد لرفع الحد: `SESSION_MINT_ENFORCE`
لا يُفعَّل إلا بعد أن يصبح الحد ≥ 106، أول بناء يرسل إثبات الجهاز
(ops/RELEASE_v1.0.61.md §5).

**الأداة:** `ops/scripts/min_build.sh` (على الـVPS، من `/root/tutor-guardian`):

```bash
ops/scripts/min_build.sh current        # الحد في .env والحد الذي يقدّمه السيرفر فعلًا
ops/scripts/min_build.sh census 30      # الأجهزة النشطة آخر 30 يومًا لكل بناء
ops/scripts/min_build.sh plan 109       # من سيُجبَر على التحديث + فحص الأمان
ops/scripts/min_build.sh apply 109      # plan ← نسخة من .env ← تعديل ← recreate ← تحقق
```

**قاعدتان لا تُكسران:**
1. **الحد ≤ آخر بناء متاح بنسبة 100% على Play.** حدٌّ فوق ما يقدّمه Play (إصدار
   ما زال في المراجعة، أو طرح تدريجي) يُغلق التطبيق على كل من تحته بلا تحديث
   يمكن تنزيله. `plan` يرفض أي حد لا يعمل عليه (أو على أعلى منه) 20 جهازًا نشطًا
   على الأقل في آخر 7 أيام (`PROOF_MIN`، `PROOF_DAYS`)، ويطبع توزيعهم على البناءات
   ليظهر إن كانوا مختبرين؛ و`FORCE=1` يتخطى الفحص، ولا يُستخدم إلا بعد التأكد من
   Play Console.
2. **القرار بالأرقام.** `plan` يطبع نسبة من سيُجبَر على التحديث، ومنهم «unknown»
   (بناء أقدم من التعداد) لأنه تحت أي حد.

**التعداد الأول (2026-09-26، آخر 30 يومًا، 964 جهازًا):** 101 = 46.0%، 105 = 39.7%،
unknown = 10.0%، 108 = 2.4%، وكل ما هو ≥ 109 أربعة أجهزة فقط (111×3 مختبرون على
internal، 110×1). لا يوجد جهاز واحد على 109.

**القرار: لا يُرفع الحد الآن.**
- **109 مرفوض:** يُجبر 960 من 964 (99.6%) على تحديث لا دليل أن Play يقدّمه لهم
  بعد، فالأجهزة الأربعة التي «أثبتته» مختبرون. النسخة الأولى من `plan` قبلت جهازًا
  واحدًا دليلًا وطبعت OK؛ صار الحد الأدنى `PROOF_MIN=20` جهازًا.
- **105 ليس بديلًا:** يُجبر 541 جهازًا (56%: 101 وما قبله وunknown) مقابل لا
  شيء، فهو لا يفتح `SESSION_MINT_ENFORCE` الذي يحتاج 106. وكل من يُجبَر الآن ينزل
  على 111، وهو بناء جديد غيّر مسار الإقلاع والإشعارات ولم يُجرَّب بعد على أجهزة
  حقيقية بهذا الحجم.

**متى يُعاد:** بعد اعتماد 111 ونشره، وبعد أيام بلا تراجع في Android vitals، يُعاد
`census` أسبوعيًا. حين يعدّي `plan 106` فحص الإثبات من تلقاء نفسه وتكون نسبة
المُجبَرين مقبولة، يُرفع الحد إلى 106 (أو أعلى إن سمح التعداد)، ثم
`SESSION_MINT_ENFORCE`. وحتى ذلك الحين يُدفع التحديث بالحسنى:
`ops/scripts/send_update_push.py` بعد استقرار 111.

**الرجوع:** `apply` يطبع أمر الرجوع إلى القيمة السابقة ويترك نسخة `.env.bak.<وقت>`.
`env_file` يُقرأ عند إنشاء الحاوية، فإعادة التشغيل وحدها لا تكفي؛ الأداة تنفّذ
`up -d --no-build --force-recreate backend`.

**بعدها:** قيدٌ في OPERATIONS_LOG (القيمة السابقة والجديدة ونتيجة `plan`)، ثم
أسبوع ومراجعة `census` قبل `SESSION_MINT_ENFORCE`.
