/// Emotion Maze Game Screen — P1.4 Improved (متاهة المشاعر 🧠).
///
...
class EmotionMazeGameScreen extends ConsumerStatefulWidget {
  const EmotionMazeGameScreen({super.key});

  @override
  ConsumerState<EmotionMazeGameScreen> createState() => _EmotionMazeGameScreenState();
}

class _EmotionMazeGameScreenState extends ConsumerState<EmotionMazeGameScreen> {
  ...
  Future<void> _loadProgress() async {
    ...
    final json = prefs.getString('game_progress_emotion_maze');
    if (json != null) {
  ...
  Future<void> _saveProgress() async {
    if (_progress != null) {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('game_progress_emotion_maze',
        _progress!.toJson().toString());
    }
  }

  void _initGame(int level) {
    final gameConfig = EmotionMazeConfig.forLevel(level);
    setState(() {
      _isGameOver = false;
      _finalScore = 0;
  ...
  void _handleGameComplete(int score, bool completed, int level) {
    ...
    _progress ??= GameProgress(gameId: 2);
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              completed
                  ? 'مستوى $level مكتمل! تعلمت التمييز بين المشاعر الإيجابية والسلبية.'
                  : 'انتهت المحاولات. تذكر: خذ نفساً عميقاً واختر الهدوء.',
        ...
        actions: [
          if (completed && level < 10)
            TextButton(
              ...
              child: const Text('المستوى التالي ▶', style: TextStyle(fontSize: 16, color: Color(0xFFA855F7))),
            ),
          TextButton(
            onPressed: () {
              Navigator.pop(ctx);
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  'تعلم تنظيم المشاعر: اضغط على الباب الذي يمثل المشاعر الهادئة والإيجابية (😌) وتجنب باب الغضب والانفعال (😡). '
                                  'كل مستوى يزداد صعوبة ويقل وقت التفكير.',
                        ...
                        children: [
                          Container(
                            ...
                            child: const Row(
                              ...
                              children: [
                                ...
                                Text('هدوء/إيجابي', style: TextStyle(fontSize: 14, color: Color(0xFF10B981), fontWeight: FontWeight.w600)),
                              ],
                            ),
                          ),
                          const SizedBox(width: 8),
                          Container(
                            ...
                            child: const Row(
                              ...
                              children: [
                                Text('😡 ', style: TextStyle(fontSize: 14)),
                                Text('غضب/سلبي', style: TextStyle(fontSize: 14, color: Color(0xFFEF4444), fontWeight: FontWeight.w600)),
                              ],
                            ),
                          ),
                        ],
...
class _LevelCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BouncyTap(
      onTap: onTap,
      child: Card(
        color: unlocked ? const Color(0xFF1E1B4B) : const Color(0xFF1E1B4B),
        shape: RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(Dt.rCard),
          side: BorderSide(
            color: unlocked
                ? (completed ? const Color(0xFF10B981) : const Color(0xFFA855F7)).withValues(alpha: 0.5)
