...
| DS5 | `AppPalette.current` is a **mutable global** set in `MaterialApp.builder`. Widgets that don't depend on `Theme` keep stale colours after a light/dark switch, until they rebuild for another reason | Flicker and stale colours on theme toggle | ✅ `AppColors` ThemeExtension (with `lerp`, so `AnimatedTheme` fades the palette) + `context.colors`; a real light/dark switch now rebuilds every element once (`AppPalette.sync`), so the ~500 `Dt.*`/`AppTheme.*` sites that do not depend on `Theme` stop keeping the old palette. Those getters remain as the shim; new code uses `context.colors`, migration continues screen by screen |
...
| DS8 | Cairo and the Quran fonts are fetched at runtime by `google_fonts`, not bundled. An offline first launch renders fallback glyphs (different Arabic shaping and metrics, causing layout shift), and each launch contacts `fonts.gstatic.com` | Offline quality, privacy | ✅ the 16 files google_fonts would fetch (Cairo ×8, Amiri Quran, Tajawal ×7; same sha256) are bundled in `assets/google_fonts/` with their OFL licences, which appear on the licences page; `allowRuntimeFetching = false`; `test/bundled_fonts_test.dart` fails if code uses an unbundled family |
| DS9 | Only 17 `Semantics` in 58k LOC, against 19 `GestureDetector`s used as buttons | Screen-reader users can't operate custom controls | ✅ `BouncyTap`/`BouncyButton` (used app-wide) are buttons with an enabled state — a disabled one no longer offers a tap action — plus the 15 remaining tap targets (coins, flashcards, tasbeeh, bedtime dhikr, tour, invite code, video play/pause, Qur'an verse and last-read, bookshelf, badges, stat chips); drawing and hold surfaces left as they are |
...
- Reduce motion: gate flutter_animate entrances on `MediaQuery.disableAnimationsOf(context)`.
  - ✅ Done for every **loop** (`core/motion.dart`: `loopUnlessReduced`, `syncLoop`) — the thinking dots, skeleton shimmer, night sky, bedtime and story pulses, path CTA. One-shot entrances need nothing: Flutter already runs every controller at 5% speed under that setting, which is also why an ungated loop turned into flicker.
  - ✅ 200% text-scale tests for the answer bubble (with follow-ups and feedback), the error view and the primary button, in Arabic and English (`test/ux1_accessibility_test.dart`).
...
| **UX-1: foundations (2–3 wks)** — ✅ fonts bundled (DS8), Semantics pass (DS9), reduce-motion loops, 200% text-scale tests; ✅ DS5 palette extension + correct theme switch; lesson reflections and coin rows readable in dark mode; 📋 remaining: literal colours/sizes on the other top screens, TalkBack walkthrough | `ThemeExtension` migration (DS5); bundled fonts (DS8); domain on-colours (DS6, DS7); replace literal colours and sizes on the 10 most-used screens; Semantics pass on the 19 `GestureDetector` buttons; reduce-motion; 200% text-scale tests | 0 contrast failures in the token test; font requests at launch = 0; TalkBack walkthrough of the 4 key journeys passes |
