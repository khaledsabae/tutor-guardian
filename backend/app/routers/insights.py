...
async def get_parenting_insights(
    ...
    # sqlite is blocking — gather everything in the threadpool, and close the
    # connection BEFORE awaiting the LLM (never hold a conn across an await).
    child, total_sleep, feed_count, feed_amount, diaper_count, chat_texts = (
        await asyncio.to_thread(_gather_insights_data, device_id, child_id)
    )
...
def _gather_insights_data(device_id: str, child_id: int):
    """Blocking sqlite reads for the insights prompt (runs via to_thread)."""
    conn = get_conn()
    try:
        ...
        # 2. Routine events summary (last 7 days)
        ...
        # 3. Recent parent questions from chat history
        chats = conn.execute(
            """
